...
export function StudentLabEditor({ lab }: StudentLabEditorProps) {
  ...
  const handleCompleteStep = () => {
    ...
    if (!completedSteps.has(currentStepIndex) && currentStepIndex < steps.length - 1) {
  ...
  const handleCheckProgress = () => {
    ...
    if (placedEquipments.length >= expectedEquipments.length) {
      score++;
      feedback.push(`✓ You have placed ${placedEquipments.length} equipment items`);
    } else {
      errors.push(`✗ Missing equipment: You have ${placedEquipments.length}/${expectedEquipments.length} items`);
    }
    ...
    const missingTypes = expectedTypes.filter(type => !placedTypes.includes(type));
    ...
    if (expectedConnections.length > 0) {
      if (wireConnections.length >= expectedConnections.length) {
        ...
        errors.push(`✗ Missing connections: You have ${wireConnections.length}/${expectedConnections.length} connections`);
      }
    } else {
      if (wireConnections.length > 0) {
        score++;
        feedback.push(`✓ You have created ${wireConnections.length} connections`);
      } else {
    ...
    if (completedSteps.size === totalSteps) {
      ...
      errors.push(`✗ Complete all steps: ${completedSteps.size}/${totalSteps} done`);
    }
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {validationResults.feedback.map((msg, idx) => (
                      <Alert key={idx} className="bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800">
                      ...
                      >
                        <CardContent className="p-3">
                          <div className="flex items-start gap-3">
                            <button
                              onClick={(e) => handleToggleStepComplete(index, e)}
              ...
              <Card>
                <CardContent className="p-4 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">Equipment Placed:</span>
                    <Badge variant="secondary">{placedEquipments.length}/{expectedEquipments.length}</Badge>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <Badge variant="secondary">{completedSteps.size}/{totalSteps}</Badge>
                  </div>
                  <Separator />
                  <Alert className="mt-2">
                    <Info className="h-4 w-4" />
                    <AlertDescription className="text-xs">
                      Click <strong>"Check Progress"</strong> to validate your lab setup and get feedback
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
